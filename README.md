...
- **跨時區時差修正**：自動計算跨時區航班飛行時間 (Duration) 與轉機停留時間 (Layover)，避免因直接相減當地時間而產生的時差誤差
...
### 3. 🗺️ 跨維度防呆與診斷 (地圖與時序校驗)
結合航班時間與住宿預定的**真實地理座標**，進行高精度的交叉檢驗。
- **時序精準地點防呆**：計算住宿與機場的距離。特別針對 Open-Jaw 行程，系統會依據**首尾日入住與退房日期**精確比對對應起降外站機場（如首日對比去程抵達機場、尾日對比回程起飛機場），徹底杜絕「回程當天住東京，卻要從大阪搭飛機」的時空錯位盲區。
- **未完成行程診斷 (Nesting Diagnostics)**：對於因資料缺漏或不連貫（如連續台灣出發）而無法正常結案的行程，系統會自動診斷並提供具體排錯引導（如「重複出發」、「缺少回程」、「只有回程」）。
- **排程防呆**：住宿時間缺口、重疊，或是航班時間衝突，皆會在列表上亮起警告。
...
| 變數名稱 | 必填 | 說明 |
|----------|------|------|
| `VITE_GOOGLE_CLIENT_ID` | ✅ **必備** | 啟用 Drive 備份與 Calendar 同步功能。若無會導致應用程式強制中斷。 |
| `VITE_GOOGLE_MAPS_API_KEY` | ✅ **必備** | 渲染核心地圖功能與 Geocoding (飯店地址自動轉座標)。 |
| `VITE_FLIGHT_PROXY_URL` | 🟢 推薦 | [重要！] 隱藏航班 API 用的 Cloudflare Worker 網址。 |
| `VITE_MAPBOX_API_KEY` | ⚪ 選配 | 作為 Geocoding 的第二備援方案。 |
